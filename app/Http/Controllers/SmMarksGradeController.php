...
class SmMarksGradeController extends Controller
{
    ...
    {
        try{
            $marks_grades = SmMarksGrade::orderBy('gpa', 'desc')->where('academic_id', YearCheck::getAcademicId())->where('school_id',Auth::user()->school_id)->get();
            // return $marks_grades;
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
         ...
         $is_duplicate = SmMarksGrade::where('school_id', Auth::user()->school_id)->where('grade_name', $request->grade_name)->where('academic_id', YearCheck::getAcademicId())->first();
         if ($is_duplicate) {
             Toastr::error('Duplicate name found!', 'Failed');
             return redirect()->back()->withErrors($validator)->withInput();
         }
         $is_duplicate_gpa = SmMarksGrade::where('school_id', Auth::user()->school_id)->where('gpa', $request->gpa)->where('academic_id', YearCheck::getAcademicId())->first();
         if ($is_duplicate_gpa) {
        ...
        try{
            ...
            $marks_grade->academic_id = YearCheck::getAcademicId();
    ...
    public function show(Request $request, $id)
    {
        try{
            ...
            $marks_grades = SmMarksGrade::where('academic_id', YearCheck::getAcademicId())->where('school_id',Auth::user()->school_id)->get();
    ...
    public function update(Request $request, $id)
    {
        ...
        // school wise uquine validation
        $is_duplicate = SmMarksGrade::where('school_id', Auth::user()->school_id)->where('academic_id', YearCheck::getAcademicId())->where('grade_name', $request->grade_name)->where('id', '!=', $request->id)->first();
        if ($is_duplicate) {
            Toastr::error('Duplicate name found!', 'Failed');
            return redirect()->back()->withErrors($validator)->withInput();
        }
        $is_duplicate_gpa = SmMarksGrade::where('school_id', Auth::user()->school_id)->where('academic_id', YearCheck::getAcademicId())->where('gpa', $request->gpa)->where('id', '!=', $request->id)->first();
        if ($is_duplicate_gpa) {
            Toastr::error('Duplicate GPA found!', 'Failed');
            return redirect()->back()->withErrors($validator)->withInput();
        }
