...
class SmSubjectAttendanceController extends Controller
{
    ...
    public function search(Request $request)
    {
        ...
        try{
            ...
            $classes = SmClass::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',Auth::user()->school_id)->get();
            $students = SmStudent::where('class_id', $class)->where('section_id', $section)->where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('school_id',Auth::user()->school_id)->get();
            ...
            foreach ($students as $student) {
                $attendance = SmSubjectAttendance::where('student_id', $student->id)->where('subject_id', $subject)->where('attendance_date', date('Y-m-d', strtotime($request->attendance_date)))->where('academic_id', YearCheck::getAcademicId())->first();
    ...
    public function storeAttendance(Request $request)
    {
        ...
        foreach ($request->id as $student) {
            $attendance = SmSubjectAttendance::where('student_id', $student)->where('subject_id', $request->subject)->where('attendance_date', date('Y-m-d', strtotime($request->date)))->where('academic_id', YearCheck::getAcademicId())->first();
    ...
    public function subjectAttendanceReportSearch(Request $request)
    {
        ...
        try{
            ...
            foreach ($students as $student) {
                ...
                ->where('sm_subject_attendances.academic_id', YearCheck::getAcademicId())
