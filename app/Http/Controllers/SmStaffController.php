    ...
    public function staffList(Request $request)
    {
        ...
        try {
            ...
            if (Auth::user()->role_id == 1) {
                ...
                $staffs = SmStaff::where('school_id', Auth::user()->school_id)->where('role_id', '!=', 1)->get();
            }
 ...
 */
        ...
        try {
            ...
            $staff->joining_letter = $joining_letter;
            $staff->other_document = $other_document;
            // $staff->academic_id = YearCheck::getAcademicId();
            $result = $staff->update();
    ...
    public function searchStaff(Request $request)
    {
        ...
        try {
            ...
            if (Auth::user()->role_id != 1) {
                $roles = InfixRole::where('active_status', '=', '1')->where('id', '!=', 1)->where('id', '!=', 2)->where('id', '!=', 3)->where('id', '!=', 5)->where(function ($q) {
                    $q->where('school_id', Auth::user()->school_id)->orWhere('type', 'System');
                })->get();
            } else {
                $roles = InfixRole::where('active_status', '=', '1')->where('id', '!=', 2)->where('id', '!=', 3)->where(function ($q) {
