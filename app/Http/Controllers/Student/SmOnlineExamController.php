...
class SmOnlineExamController extends Controller
{
    ...
    public function studentOnlineExam()
    {
        try {
            ...
            $online_exams = SmOnlineExam::where('active_status', 1)->where('academic_id', YearCheck::getAcademicId())->where('status', 1)->where('class_id', $student->class_id)->where('section_id', $student->section_id)->where('school_id', Auth::user()->school_id)->get();
            ...
            $marks_assigned = [];
            foreach ($online_exams as $online_exam) {
                $exam = SmStudentTakeOnlineExam::where('online_exam_id', $online_exam->id)->where('academic_id', YearCheck::getAcademicId())->where('student_id', $student->id)->where('status', 2)->where('school_id', Auth::user()->school_id)->first();
                if ($exam != "") {
    ...
    public function takeOnlineExam($id)
    {
        try {
            ...
            $assigned_questions = SmOnlineExamQuestionAssign::where('online_exam_id', $online_exam->id)->where('academic_id', YearCheck::getAcademicId())->where('school_id', Auth::user()->school_id)->get();
    ...
    public function studentOnlineExamSubmit(Request $request)
    {
        ...
        try {
            ...
            foreach ($request->question_ids as $question_id) {
                ...
                $exam_question->academic_id =  YearCheck::getAcademicId();
                $exam_question->save();
                $exam_question->toArray();
                if ($question_bank->type == "M") {
                    $question_options = SmQuestionBankMuOption::
                    ...
                    foreach ($question_options as $question_option) {
                        ...
                        $exam_question_option->academic_id =  YearCheck::getAcademicId();
