...
class SmLeaveTypeController extends Controller
{
    ...
    {
        try{
            $leave_types = SmLeaveType::where('school_id',Auth::user()->school_id)->where('academic_id', YearCheck::getAcademicId())->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
         ...
         $is_duplicate = SmLeaveType::where('school_id', Auth::user()->school_id)->where('type', $request->type)->where('academic_id', YearCheck::getAcademicId())->first();
         if ($is_duplicate) {
        ...
        try{
            ...
            $leave_type->academic_id = YearCheck::getAcademicId();
    ...
    public function update(Request $request, $id)
    {
        ...
        // school wise uquine validation
        $is_duplicate = SmLeaveType::where('school_id', Auth::user()->school_id)->where('type', $request->type)->where('id', '!=', $request->id)->where('academic_id', YearCheck::getAcademicId())->first();
        if ($is_duplicate) {
            Toastr::error('Duplicate name found!', 'Failed');
            return redirect()->back()->withErrors($validator)->withInput();
        }
