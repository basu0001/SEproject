...
class CustomResultSetting extends Model
{
    public static function getGpa($marks){
        try {
            $marks_gpa=DB::table('sm_marks_grades')->where('percent_from','<=',$marks)->where('percent_upto','>=',$marks)->where('academic_id', YearCheck::getAcademicId())->first();
    ...
    public static function gpaToGrade($gpa){
        try {
            $marks_gpa=DB::table('sm_marks_grades')->where('from','<=',$gpa)->where('up','>=',$gpa)->where('academic_id', YearCheck::getAcademicId())->first();
                return $marks_gpa->grade_name;
        } catch (\Exception $e) {
            $data=[];
            return $data;
